...
**Initial paths** are computed identically to Solo mode (minimise `distance / speedLimit`), plus each segment's expected event delay. No congestion penalty applies yet because no trains have departed.
