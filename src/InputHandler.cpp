...
/*   Updated: 2024/10/24 10:53:56 by andrefranci      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void InputHandler::loadRailNetworkData()
{
	...
	// the first word of each line should be "Node", "Event", or "Rail" not case
	// sensitive if the first word is not one of the above, throw an exception
	// also if the first word is "Node", the line should have 2 words, the first
	// one is "Node" and the second one is the node name if the first word is
	// "Event", the line should have 5 words, the first one is "Event", the
	// second one is the event name(it can be a single word or group of word
	// inside quotes ex: Riot, or "There is something"), and the third is a
	// double type reenting  probability, the fourth duration of the event in
	// time units, and the fifth is the node name if the first word is "Rail",
	// the line should have 4 words, the first one is "Rail", the second one is
	// where the rail starts, the third is where the rail ends, and the fourth
	// is the length of the rail in time units in double type
	...
	while (std::getline(file, line))
	{
		...
		if (word == "Node")
		{
			if (nodeStr)
			{
				throw InputException("Invalid file format: "
									 + railNetworkFilePath);
			}
			nodeStr = true;
			std::string nodeName;
